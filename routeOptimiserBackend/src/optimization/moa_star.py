...
class MOAStar:
    """Multi-objective A* with Pareto pruning.

    The objective vector is declared by OBJECTIVE_KEYS and the per-edge cost accumulation
    lives in _edge_costs(), so a subclass can optimise a different set of objectives over a
    different graph (see src/optimization/ner_router.py) while reusing this search
    unchanged. The defaults below are the original cross-border behaviour and must stay
    exactly as they are.
    """

    OBJECTIVE_KEYS = ("time", "cost", "emissions", "customs")
    ...
    def _edge_costs(self, current, neighbor, edge_data, costs, weight_kg):
        """Accumulate the objective vector across one edge. Override to change objectives."""
        ...
        return (new_time, new_cost, new_emissions, new_customs)

    def _metrics_dict(self, costs):
        """Label the objective vector for the response payload."""
        return {key: costs[i] for i, key in enumerate(self.OBJECTIVE_KEYS)}
    ...
    def moa_star(self, start, goal, weights, weight_kg, max_days):
        ...
        # (f_score, node, path, costs) where costs is the objective vector declared by
        # OBJECTIVE_KEYS — (time, cost, emissions, customs) by default.
        open_set = [(0, start, [start], tuple(0 for _ in self.OBJECTIVE_KEYS))]
        closed_set = set()
        pareto_frontier = {}
        ...
        while open_set:
            ...
            if current == goal:
                ...
                if total_time_days <= max_days:
                    ...
                    return path, self._metrics_dict(costs)
                else:
            ...
            for neighbor, edge_data_dict in self.G[current].items():
                for edge_key, edge_data in edge_data_dict.items():
                    ...
                    new_costs = self._edge_costs(current, neighbor, edge_data, costs, weight_kg)
